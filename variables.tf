...
variable "acm_certificate_arn" {
  description = "The ARN of an existing ACM certificate to attach to the ALB Listener."
  type        = string
...
variable "alb_sg_ingress_rules" {
  description = "A map of ingress security group rules for the ALB security group. Each key is a unique identifier for the rule, and the value is a map containing the rule's details."
  type = map(object({
...
variable "alb_sg_egress_rules" {
  description = "A map of egress security group rules for the ALB security group. Each key is a unique identifier for the rule, and the value is a map containing the rule's details."
  type = map(object({
    from_port                    = optional(number)
    to_port                      = optional(number)
    ip_protocol                  = optional(string)
